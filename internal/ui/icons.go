...
// Icon is one app's glyph identity: a Nerd Font name (the source-of-truth
// reference for the codepoint), the rune itself, and the color it renders
// in.
type Icon struct {
	Name  string
	Glyph string
...
// Icons (JetBrainsMono Nerd Font, the Omarchy default). Glyphs are fixed
// (verified codepoints); COLORS come from the active theme via applyTheme,
// so nothing here is a hardcoded hex. The colors are assigned once at
// startup, so these stay plain package vars rather than being threaded
...
// applyTheme fills every icon's Color from the loaded palette. claude gets
// orange and other agents blue (that's how a row flags "this is an agent"
// at a glance); the rest map to the language/tool's conventional theme slot.
// Call once, before the delegate is built.
...
// buildAppIcons is the app->Icon table. A flat map, not an abstraction --
// one lookup, one fallback, no interface. Built after applyTheme colors the
// icons so the map captures their themed values.
...
// iconFor looks up an app's Icon, falling back to a generic glyph for
